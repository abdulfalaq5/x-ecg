...
<body>
            ...
            <div class="col content-page p-5 bg-white">
                <nav aria-label="Breadcrumb" class="mb-2">
                    <ol class="breadcrumb">
                        ...
                        <li><a href="#">Detail Quiz</a></li>
                    </ol>
                </nav>
                <div class="container mt-5">
                    <div class="row">
                        <div class="col-md-12">
                            <h2 class="mb-4">Detail Quiz</h2>
                            <div class="row">
                                <div class="col-md-6">
                                    <label class="mb-4">Title</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">: {{ $data_quiz->title_quiz }}</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">Star Date</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">: {{ $data_quiz->waktu_quiz }}</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">End Date</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">: {{ $data_quiz->waktu_akhir_quiz }}</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">Description</label>
                                </div>
                                <div class="col-md-6">
                                    <label class="mb-4">: {{ $data_quiz->des_quiz }}</label>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <h2 class="mb-4">List Question Quiz</h2>
                            <a class="btn btn-success"
                                href="{{ route('editor.quiz.detail.add', [$data_quiz->id, $id, $materi, $module]) }}"
                                style="color: #000000">Add Question Quiz</a>
                            <a class="btn btn-success"
                                href="{{ route('editor.quiz.detail.import', [$data_quiz->id, $id, $materi, $module]) }}"
                                style="color: #000000">Import Question Quiz</a><br>
                            <table class="table table-bordered table-hover">
                                ...
                                @if (!empty($data_quiz_detail))
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($data_quiz_detail as $item)
                                        <tr>
                                            <td>{{ $no++ }}</td>
                                            <td>{{ $item['jenis'] == 2 ? 'Multiple choice' : 'True or False' }}</td>
                                            <td>{{ $item['pertanyaan'] }}</td>
                                            <td>{{ $item['bobot_nilai'] }}</td>
                                            <td>
                                                <a href="{{ route('editor.quiz.detail.delete', [$item['id'], $data_quiz->id, $id, $materi, $module]) }}"
                                                    onclick='return confirm("Are you sure To delete this");'
                                                    class="btn btn-primary" style="color: black">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
    ...
    <script>
        $("#form-data").submit(function(e) {
            ...
            if (title_materi == '') {
                alert('Module name must be filled in');
                e.preventDefault();
                return false;
            }
